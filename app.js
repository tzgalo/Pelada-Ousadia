...
async function boot() {
  ...
  if(fbStore==='local') {
    ...
    // Load from Firestore
    const savedCfg = LS.get('fbcfg') || JSON.parse(fbStore);
    try {
      // Re-init with saved config
      if(savedCfg.apiKey!=='REPLACE_API_KEY') {
...
function renderFlowSel(fc) {
  ...
  html += all.map(j=>{
    ...
    return `<div class="check-row" onclick="toggleSel('${j.id}')">
      <div class="check-box ${checked?'checked'}" id="chk-${j.id}"></div>
      <div class="player-avatar sm">${j.foto?`<img src="${j.foto}"/>`:initials(j.nome)}</div>
      <span class="player-name">${escHtml(j.nome)}</span>
    </div>`;
  }).join('');
